"""Jawaban w10 — SOLUSI (untuk dosen/validasi)
"""

from __future__ import annotations

def q01() -> bool:
    """[T/F] Teorema Limit Pusat menyatakan bahwa rata-rata sampel akan mendekati
distribusi Normal jika ukuran sampel cukup besar ( 0)."""
    return True

def q02() -> bool:
    """[T/F] Rata-rata dari distribusi sampling rata-rata ($\bar{X}$) selalu sama dengan rata-rata
populasi ."""
    return True

def q03() -> bool:
    """[T/F] Semakin besar ukuran sampel, semakin besar pula Standard Error-nya."""
    return False

def q04() -> str:
    """[MC] Jika populasi memiliki $\sigma = 10$ dan ukuran sampel $n = 100$, maka Standard Error (SE)
adalah:

a) 10
b) 1
c) 0,1
d) 100"""
    return "B"

def q05() -> str:
    """[MC] Distribusi sampling dari proporsi sampel p akan mendekati Normal jika:
a)  sangat kecil.
b) p 5 dan 1 −p 5.
c) Populasi sangat heterogen.
d) p = 0, 5."""
    return "B"

def q06() -> str:
    """[MC] Faktor  dalam rumus Standard Error menunjukkan bahwa:
a) Error berkurang secara linear dengan .
b) Untuk mengurangi error menjadi setengahnya, kita butuh sampel 4 kali lebih banyak.
c) Error tidak dipengaruhi oleh .
d) Sampel besar selalu buruk."""
    return "B"

def q07() -> str:
    """[MC] Statistik sampel yang digunakan untuk menduga parameter populasi disebut:
a) Estimator.
b) Variabel.
c) Konstanta.
d) Bias."""
    return "A"

def q08() -> float:
    """[Numeric] Jika = 50 dan = 10, berapakah nilai rata-rata dari distribusi sampling rata-
rata untuk = 5?"""
    return 50

def q09() -> float:
    """[Numeric] Hitung Standard Error jika = 1 dan = 6."""
    return 2

def q10() -> float:
    """[Numeric] Jika rata-rata populasi 100 dan Standard Error 2, berapakah skor-Z untuk
rata-rata sampel = 10?"""
    return 2

def q11() -> float:
    """[Numeric] Jika ukuran sampel adalah 64 dan simpangan baku populasi 8, berapakah
simpangan baku distribusi rata-rata sampel?"""
    return 1

def q12() -> float:
    """[Numeric] Berapakah nilai rata-rata sampel  jika skor-Z nya adalah 1,5, = 0, dan
Standard Error = 4?"""
    return 86
